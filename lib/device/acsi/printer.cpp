...
constexpr const char * const ACSIPrinter::printer_model_str[PRINTER_INVALID];

// Constructor just sets a default printer type
ACSIPrinter::ACSIPrinter(FileSystem *filesystem, printer_type print_type)
{
    _storage = filesystem;
    set_printer_type(print_type);
}

ACSIPrinter::~ACSIPrinter()
{
    //vTaskDelete(thPrinter);
    delete _pptr;
}

ACSIPrinter::printer_type ACSIPrinter::match_modelname(std::string model_name)
{
    int i;
    for (i = 0; i < PRINTER_INVALID; i++)
        if (model_name.compare(ACSIPrinter::printer_model_str[i]) == 0)
            break;
...
void ACSIPrinter::shutdown()
{
}

void ACSIPrinter::set_printer_type(printer_type printer_type)
{
